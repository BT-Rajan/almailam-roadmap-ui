export default {
  pageTitle: 'الإدارة',
  pageSubtitle: 'تهيئة وإدارة سيرفس أو إس على مستوى الشركة.',

  userRole: {
    administrator: 'مسؤول النظام',
    projectManager: 'مدير مشروع',
    engineer: 'مهندس',
    documentController: 'مراقب مستندات',
    viewer: 'مُشاهد',
  },
  userStatus: {
    active: 'نشط',
    inactive: 'غير نشط',
  },

  aiPage: {
    pageTitle: 'ذكاء المعرفة الاصطناعي',
    pageSubtitle: 'إعداد المزود، موجه التأصيل، والحدود الخاصة بمساعد الأسئلة والأجوبة المعرفي.',
    availability: 'التوفر',
    availabilityDescription:
      'أداة الأسئلة والأجوبة المعرفية هي الميزة الوحيدة المعتمدة على الذكاء الاصطناعي إلى جانب التحقق من هوية العميل. تستمر جميع سير العمل الأخرى بشكل طبيعي إذا تم تعطيلها.',
    enableAssistant: 'تفعيل المساعد المعرفي',
    enableAssistantHint:
      'يسري فورًا — يخفي أيقونة الشرارة/درج المحادثة ولوحة السؤال، ويعطّل نقطة نهاية السؤال، عند إيقافه.',
    providerSelection: 'اختيار المزود',
    providerSelectionDescription: 'اختر المزود الافتراضي وترتيب أولوية الاحتياط.',
    defaultProvider: 'المزود الافتراضي',
    providerPriority: 'أولوية المزودين',
    default: 'افتراضي',
    moveUp: 'تحريك لأعلى',
    moveDown: 'تحريك لأسفل',
    modelPerformance: 'النموذج والأداء',
    modelPerformanceDescription: 'يُطبَّق على كل طلب أسئلة وأجوبة معرفي.',
    timeoutSeconds: 'المهلة (ثوانٍ)',
    maxTokens: 'الحد الأقصى للرموز',
    temperature: 'درجة العشوائية',
    cachingRetries: 'التخزين المؤقت وإعادة المحاولة',
    cachingRetriesDescription: 'يُخدَّم السؤال المتكرر على نفس المستند (المستندات) من الذاكرة المؤقتة بدلاً من استدعاء المزود مجددًا.',
    answerCacheDuration: 'مدة التخزين المؤقت للإجابة (دقائق)',
    answerCacheDurationHint: 'القيمة 0 تُعطّل التخزين المؤقت.',
    retryLimit: 'حد إعادة المحاولة',
    documentLimits: 'حدود المستندات',
    documentLimitsDescription: 'حدود ما يمكن رفعه وكمية نص المستند المُرسلة إلى المزود لكل سؤال.',
    maxUploadSize: 'الحد الأقصى لحجم الرفع (ميغابايت)',
    maxCharsPerDocument: 'الحد الأقصى للأحرف لكل مستند',
    maxContextChars: 'الحد الأقصى لأحرف السياق (جميع المستندات)',
    maxContextCharsHint: 'يحدّ إجمالي نص المستندات المُرسل لكل سؤال عند السؤال عبر جميع المستندات النشطة.',
    groundingPrompt: 'موجّه التأصيل',
    groundingPromptDescription:
      'يوجّه النموذج للإجابة حصريًا من المستند (المستندات) المرفوعة، والبقاء ملموسًا بدلاً من عام، والحفاظ على نبرة حازمة ومهذبة، والرد بلغة الزائر (العربية أو الإنجليزية أو مزيج منهما). عدّل بحذر.',
    resetToDefault: 'إعادة الضبط الافتراضي',
    saveChanges: 'حفظ التغييرات',
    knowledgebaseAssistant: 'المساعد المعرفي',
    enabled: 'مفعّل',
    disabled: 'معطّل',
    timeoutLabel: 'المهلة',
    cacheDurationLabel: 'مدة التخزين المؤقت',
    minutesAbbrev: 'د',
    summaryFooter: 'يجيب على الأسئلة حصريًا من المستندات المرفوعة. تستمر جميع سير العمل الأخرى بشكل طبيعي إذا تم تعطيله.',
    modelLine: 'النموذج: {model}',
    serverDefault: 'افتراضي الخادم',
    currentKey: 'المفتاح الحالي: {key}',
    keyUnreadable: 'لا يمكن قراءة المفتاح المحفوظ لهذا المزود بعد الآن (مثلاً بعد تغيير مفتاح التشفير على الخادم). يرجى إعادة إدخاله.',
    apiKey: 'مفتاح API',
    apiKeyPlaceholder: 'أدخل مفتاح API جديدًا للتحديث',
    updateKey: 'تحديث المفتاح',
    stagedNotice: 'تم التجهيز — اضغط حفظ التغييرات أدناه لتخزينه (مشفّرًا) وتفعيله. لا حاجة لإعادة التشغيل.',
    testConnection: 'اختبار الاتصال',
    connected: 'متصل',
    error: 'خطأ',
    notConfigured: 'غير مُهيّأ',
    assistantEnabledTitle: 'تم تفعيل مساعد قاعدة المعرفة',
    assistantDisabledTitle: 'تم إيقاف مساعد قاعدة المعرفة',
    takesEffectImmediately: 'يسري هذا فورًا.',
    unableToSave: 'تعذّر الحفظ',
    aiConfigurationSavedTitle: 'تم حفظ إعدادات الذكاء الاصطناعي',
    aiConfigurationSavedDescription: 'تم تطبيق تغييراتك.',
    connectionSuccessfulTitle: 'نجح الاتصال',
    connectionFailedTitle: 'فشل الاتصال',
  },

  auditLogPage: {
    pageTitle: 'سجل التدقيق',
    pageSubtitle: 'سجل بكل تغيير تم تتبعه في النظام، من قام به، ومتى.',
    exportCsv: 'تصدير CSV',
    clearFilter: 'مسح عامل التصفية',
    noAuditEventsFound: 'لا توجد أحداث تدقيق',
    noAuditEventsFoundDescription: 'جرّب عامل تصفية مختلفًا لنوع الكيان، أو تحقق لاحقًا بعد إجراء المزيد من التغييرات.',
    columnEvent: 'الحدث',
    columnEntityType: 'نوع الكيان',
    columnEntityId: 'معرّف الكيان',
    columnChangedBy: 'غيّره',
    columnChangedAt: 'وقت التغيير',
    columnReason: 'السبب',
    entityAll: 'جميع الكيانات',
    entityClients: 'العملاء',
    entityProjects: 'المشاريع',
    entityDocuments: 'المستندات',
    entityTasks: 'المهام',
    entityQuotations: 'عروض الأسعار',
    entityContracts: 'العقود',
    entityFinancialAgreements: 'خطط الدفع',
    entityGovernmentAuthorities: 'الجهات الحكومية',
    entityGovernmentForms: 'النماذج الحكومية',
    entityGovernmentSubmissions: 'طلبات التصاريح',
    entityUsers: 'المستخدمون',
    entityWorkflowTemplates: 'قوالب سير العمل',
    entityCompanySettings: 'إعدادات الشركة',
    entityAiConfiguration: 'إعدادات الذكاء الاصطناعي',
    entityEmailSettings: 'إعدادات البريد الإلكتروني',
    entityScheduledReports: 'التقارير المجدولة',
    entityProjectTimeline: 'الجدول الزمني للمشروع',
    exportFailedTitle: 'فشل التصدير',
    exportFailedDescription: 'تعذّر تصدير سجل التدقيق. يرجى المحاولة مرة أخرى.',
  },

  catalogsPage: {
    pageTitle: 'الكتالوجات',
    servicesTab: 'الخدمات',
    activitiesTab: 'كتالوج الأنشطة الإضافية',
    permitsTab: 'كتالوج التصاريح',
    servicesSubtitle: 'إعداد الخدمات المقدَّمة، مقسَّمة إلى فرعي التصميم (رسم لمرة واحدة) والإشراف (شهري، بالتناسب اليومي)، وأنشطة كل خدمة وتكاليفها.',
    activitiesSubtitle:
      'إعداد فئات نوع التكليف (تصميم، إشراف، إلخ) المتاحة عند إنشاء المشروع، وقائمة الأنشطة الإضافية المسعّرة لكل فئة.',
    permitsSubtitle: 'إعداد التصاريح التي يمكن إرفاقها بمشروع أثناء الإعداد.',
    documentsTab: 'متطلبات المستندات',
    documentsSubtitle: 'حدد المستندات المطلوبة عادةً لأنشطة التصميم أو التصاريح أو الإشراف، وأعد استخدام نفس المستند لأكثر من نشاط. للعلم فقط — تظهر كقائمة مرجعية في تبويبات المشروع ولا تُفرض أبدًا.',
    tabsAriaLabel: 'أقسام الكتالوج',
  },

  documentRequirements: {
    description: 'حدد المستندات المطلوبة عادةً لأنشطة التصميم أو التصاريح أو الإشراف — يمكن ربط مستند واحد بعدة أنشطة. للعلم فقط: هذا لا يمنع إغلاق أي مهمة أو نشاط أبدًا، بل يظهر فقط كقائمة مرجعية في تبويبات المشروع.',
    name: 'اسم المستند',
    descriptionPlaceholder: 'الوصف (اختياري)',
    remove: 'إزالة',
    linkedTo: 'الأنشطة/التصاريح المرتبطة',
    linkedToDescription: 'أنشطة التصميم أو التصاريح أو أنشطة الإشراف التي يُحتاج إليها هذا المستند عادةً من أجلها.',
    noLinksYet: 'غير مرتبط بأي شيء بعد.',
    linkPlaceholder: 'اختر نشاطًا أو ترخيصًا',
    link: 'ربط',
    unlink: 'إزالة الرابط',
    noRequirementsYet: 'لا توجد متطلبات مستندات بعد — أضف واحدًا أدناه.',
    addRequirement: 'إضافة متطلب مستند',
    add: 'إضافة',
    failedToAdd: 'فشلت إضافة متطلب المستند',
    failedToRemove: 'فشلت إزالة متطلب المستند',
    failedToLoadLinks: 'فشل تحميل الأنشطة/التصاريح المرتبطة',
    failedToLink: 'فشل ربط متطلب المستند',
    failedToUnlink: 'فشلت إزالة الرابط',
  },

  adminDocumentsPage: {
    pageTitle: 'المستندات',
    formsTab: 'النماذج الحكومية',
    serviceMapTab: 'خريطة مستندات الخدمة',
    formsSubtitle: 'إدارة الجهات والنماذج ومتطلبات مستنداتها.',
    serviceMapSubtitle:
      'لكل خدمة، أي النماذج/الاتفاقيات الحكومية القابلة للتعبئة التي يحتاجها المشروع. يقوم الموظفون بتعبئتها من المشروع نفسه (الموافقات والتصاريح) — هذا يتحكم فقط بأيها متاح هناك.',
    templatesTab: 'قوالب عروض الأسعار والعقود',
    templatesSubtitle: 'ارفع قوالب .docx المستخدمة لإنشاء مستندات عروض الأسعار والعقود، واختر القالب الافتراضي لكل منها.',
    tabsAriaLabel: 'أقسام المستندات',
  },

  companyPage: {
    pageTitle: 'إعدادات الشركة',
    pageSubtitle: 'إدارة الملف التعريفي للشركة والهوية البصرية وتفضيلات الأعمال.',
    companyProfile: 'الملف التعريفي للشركة',
    companyProfileDescription: 'التفاصيل الأساسية الظاهرة في عروض الأسعار والعقود والتقديمات الحكومية.',
    companyName: 'اسم الشركة',
    tagline: 'الشعار النصي',
    tradeLicenseNumber: 'رقم الرخصة التجارية',
    website: 'الموقع الإلكتروني',
    email: 'البريد الإلكتروني',
    phone: 'الهاتف',
    address: 'العنوان',
    addressDescription: 'عنوان المكتب المسجل المستخدم في المستندات الرسمية.',
    city: 'المدينة',
    country: 'الدولة',
    branding: 'الهوية البصرية',
    brandingDescription: 'تُطبَّق على ترويسات التقارير والبوابات والمستندات القابلة للطباعة.',
    brandColor: 'لون العلامة التجارية',
    companyLogo: 'شعار الشركة',
    companyLogoDescription:
      'قابل للإدراج في أي قالب مستند عرض سعر/عقد عبر حقل دمج شعار الشركة. يُطبَّق فورًا — ليس جزءًا من "حفظ التغييرات" أدناه.',
    uploading: 'جارٍ الرفع…',
    uploadDifferentLogo: 'ارفع شعارًا مختلفًا لاستبداله',
    pngOrJpeg: 'PNG أو JPEG',
    applicationPreferences: 'تفضيلات التطبيق',
    applicationPreferencesDescription: 'الإعدادات الافتراضية المطبقة عبر التطبيق.',
    defaultLanguage: 'اللغة الافتراضية',
    timezone: 'المنطقة الزمنية',
    dateFormat: 'تنسيق التاريخ',
    currency: 'العملة',
    businessSettings: 'إعدادات الأعمال',
    businessSettingsDescription: 'الشروط الافتراضية المطبقة على عروض الأسعار الجديدة.',
    defaultPaymentTerms: 'شروط الدفع الافتراضية (أيام)',
    defaultQuotationValidity: 'صلاحية عرض السعر الافتراضية (أيام)',
    projectAlerts: 'تنبيهات المشروع',
    projectAlertsDescription: 'إخطار المهندس المكلَّف عندما يتوقف المشروع عن التقدم.',
    staleProjectAlertThreshold: 'حد تنبيه ركود المشروع (أيام)',
    staleProjectAlertHint: 'يُرسل إشعار بمجرد عدم تقدم مرحلة سير عمل المشروع لهذا العدد من الأيام.',
    onboardingAlerts: 'تنبيهات التهيئة',
    onboardingAlertsDescription: 'إخطار مدير الحساب عندما يتوقف تهيئة العميل عن التقدم.',
    staleOnboardingAlertThreshold: 'حد تنبيه ركود التهيئة (أيام)',
    staleOnboardingAlertHint: 'يُرسل إشعار بمجرد عدم تقدم حالة تهيئة العميل لهذا العدد من الأيام.',
    siteEngineerStatusReports: 'تقارير حالة مهندس الموقع',
    siteEngineerStatusReportsDescription: 'من يُختار هنا يراجع كل تقرير حالة ميداني وارد ويرفقه بالمشروع المعني.',
    statusReportRecipient: 'مستلم تقرير الحالة',
    noOneAssigned: 'لم يُعيَّن أحد',
    saveChanges: 'حفظ التغييرات',
    languageEnglish: 'الإنجليزية',
    languageArabic: 'العربية',
    logoUpdatedTitle: 'تم تحديث الشعار',
    logoUpdatedDescription: 'أصبح شعار الشركة متاحًا الآن لإدراجه في قوالب المستندات.',
    failedToUploadLogo: 'فشل رفع الشعار',
    logoRemovedTitle: 'تمت إزالة الشعار',
    failedToRemoveLogo: 'فشلت إزالة الشعار',
    companySettingsSavedTitle: 'تم حفظ إعدادات الشركة',
    companySettingsSavedDescription: 'تم تطبيق تغييراتك.',
  },

  emailPage: {
    pageTitle: 'إعدادات البريد الإلكتروني',
    pageSubtitle: 'تهيئة صندوق SMTP المستخدَم لإرسال رسائل عروض الأسعار والعقود عبر البريد الإلكتروني.',
    tabsAriaLabel: 'أقسام إعدادات البريد الإلكتروني',
    settingsTab: 'الإعدادات',
    templatesTab: 'القوالب',
    templatesSubtitle: 'تحرير عنوان ونص كل رسالة بريد إلكتروني تلقائية يرسلها التطبيق.',
    envOverrideNotice: 'تم تعيين SMTP_HOST على مستوى الخادم في البيئة، وله الأولوية على هذه الإعدادات إلى أن تتم إزالته.',
    mailbox: 'صندوق البريد',
    mailboxDescription: 'صندوق البريد الذي تُرسَل منه الرسائل عند إرسال عرض سعر أو عقد إلى عميل.',
    enableMailbox: 'تفعيل صندوق البريد هذا',
    enableMailboxHint: 'عند الإيقاف، يعود الإرسال إلى SMTP_HOST على مستوى الخادم إن وُجد، وإلا يصبح غير متاح.',
    provider: 'المزوِّد',
    smtpHost: 'مضيف SMTP',
    smtpPort: 'منفذ SMTP',
    useTls: 'استخدام TLS (STARTTLS)',
    useTlsHint: 'اترك هذا الخيار مفعَّلًا ما لم يتطلب مزوِّدك اتصالًا غير مشفَّر صراحةً.',
    useTlsLockedHint: 'هذا الإعداد ثابت لهذا المزوِّد. اختر "مخصص / آخر" لتحديد مضيف أو منفذ أو تشفير مختلف.',
    username: 'اسم المستخدم',
    usernamePlaceholder: 'عادةً نفس عنوان "من"',
    password: 'كلمة المرور',
    passwordSaved: 'كلمة المرور (محفوظة)',
    passwordPlaceholder: 'أدخل كلمة مرور جديدة للتحديث',
    fromEmail: 'عنوان "من"',
    fromName: 'اسم "من"',
    connection: 'الاتصال',
    connectionDescription: 'يحفظ تغييراتك، ثم يختبر الاتصال بتسجيل دخول SMTP فعلي.',
    testConnection: 'حفظ واختبار الاتصال',
    connected: 'متصل',
    notConnected: 'غير متصل',
    lastTested: 'آخر اختبار {time}',
    testResultOk: 'تم الاتصال بنجاح.',
    connectionSuccessful: 'نجح الاتصال',
    connectionFailed: 'فشل الاتصال',
    saveChanges: 'حفظ التغييرات',
    settingsSaved: 'تم حفظ إعدادات البريد الإلكتروني',
    settingsSavedHint: 'تم تطبيق تغييراتك.',
    unableToSave: 'تعذر الحفظ',
    pleaseTryAgain: 'يرجى المحاولة مرة أخرى.',
  },

  emailTemplates: {
    emptyTitle: 'لم يتم اختيار قالب',
    emptyDescription: 'اختر قالبًا من القائمة لتحرير عنوانه ونصه.',
    subjectLabel: 'الموضوع',
    bodyLabel: 'النص',
    availableFields: 'الحقول المتاحة -- انقر للنسخ',
    lastUpdated: 'آخر تحديث بواسطة {by} بتاريخ {at}',
    tokenCopiedTitle: 'تم النسخ إلى الحافظة',
    savedTitle: 'تم حفظ القالب',
    savedDescription: 'ستستخدم الرسالة هذه الصياغة من الآن فصاعداً.',
    failedToSave: 'فشل حفظ القالب',
    keys: {
      projectCreated: {
        label: 'إنشاء مشروع',
        description: 'تُرسل إلى العميل عند إنشاء مشروع جديد له.',
      },
      requirementConfirmed: {
        label: 'المتطلبات: تم تأكيد النطاق',
        description: 'تُرسل بمجرد تأكيد العميل لنطاق العمل.',
      },
      quotationApproved: {
        label: 'عرض السعر: تم الاعتماد',
        description: 'تُرسل بمجرد اعتماد العميل لعرض السعر -- وتتضمن نسخة منه وخطة الدفع.',
      },
      contractSigned: {
        label: 'العقد: تم التوقيع',
        description: 'تُرسل بمجرد توقيع العميل للعقد -- وتتضمن نسخة منه وملخصه.',
      },
      permitApplicationSubmitted: {
        label: 'التصريح: تم تقديم الطلب',
        description: 'تُرسل إلى العميل عند تقديم طلب تصريح إلى الجهة المختصة.',
      },
      permitResponseReceived: {
        label: 'التصريح: تم استلام الرد',
        description: 'تُرسل إلى العميل عند موافقة الجهة المختصة على طلب التصريح أو رفضه.',
      },
      paymentReceived: {
        label: 'الدفعة: إيصال استلام',
        description: 'تُرسل إلى العميل لتأكيد استلام دفعة.',
      },
      paymentReminder: {
        label: 'الدفعة: تذكير',
        description: 'تُرسل إلى العميل قبل يومين من تاريخ استحقاق الدفعة.',
      },
    },
  },

  passwordResetDialog: {
    title: 'إعادة تعيين كلمة المرور',
    heading: 'إعادة تعيين كلمة مرور {name}',
    shareNotice: 'شارك كلمة المرور الجديدة هذه مع المستخدم بأمان. لن تُعرض مرة أخرى.',
    copied: 'تم النسخ',
    copyPassword: 'نسخ كلمة المرور',
  },

  userDialog: {
    editTitle: 'تعديل المستخدم',
    addTitle: 'إضافة مستخدم',
    fullName: 'الاسم الكامل',
    fullNamePlaceholder: 'مثال: سارة عبدالله',
    salutation: 'اللقب',
    salutationPlaceholder: 'اختر اللقب',
    salutationNone: 'غير محدد',
    salutationMr: 'السيد',
    salutationMs: 'السيدة',
    designation: 'المسمى الوظيفي',
    designationPlaceholder: 'مثال: مراقب مستندات',
    email: 'البريد الإلكتروني',
    emailPlaceholder: "name{'@'}almailam.ae",
    mobile: 'الجوال',
    mobilePlaceholder: '+965 5XXX XXXX',
    role: 'الدور',
    rolePlaceholder: 'اختر الدور',
    active: 'نشط',
    activeHint: 'لا يمكن للمستخدمين غير النشطين تسجيل الدخول.',
    saveChanges: 'حفظ التغييرات',
    addUser: 'إضافة مستخدم',
    nameRequired: 'الاسم مطلوب.',
    emailRequired: 'البريد الإلكتروني مطلوب.',
    emailInvalid: 'الرجاء إدخال بريد إلكتروني صحيح.',
    roleRequired: 'الرجاء اختيار دور.',
    cannotChangeOwnRole: 'لا يمكنك تغيير دورك الخاص.',
    cannotDeactivateOwnAccount: 'لا يمكنك إلغاء تفعيل حسابك الخاص.',
  },

  authorityFormDialog: {
    editTitle: 'تعديل الجهة',
    addTitle: 'إضافة جهة',
    authorityName: 'اسم الجهة',
    authorityNamePlaceholder: 'مثال: بلدية الكويت',
    category: 'الفئة',
    website: 'الموقع الإلكتروني',
    websitePlaceholder: 'https://',
    description: 'الوصف',
    saveAuthority: 'حفظ الجهة',
  },

  governmentFormDialog: {
    editTitle: 'تعديل النموذج الحكومي',
    addTitle: 'إضافة نموذج حكومي',
    titleRequired: 'عنوان النموذج مطلوب',
    formCodeRequired: 'رمز النموذج مطلوب',
    authorityRequired: 'يرجى اختيار جهة',
    descriptionRequired: 'الوصف مطلوب',
    formTitle: 'عنوان النموذج',
    formCode: 'رمز النموذج',
    formCodePlaceholder: 'مثال: MUN-BP-01',
    authority: 'الجهة',
    category: 'الفئة',
    language: 'اللغة',
    version: 'الإصدار',
    versionPlaceholder: 'v1.0',
    lastUpdated: 'آخر تحديث',
    description: 'الوصف',
    requiredDocuments: 'المستندات المطلوبة',
    requiredDocumentsHint: 'مستند واحد في كل سطر.',
    templateContent: 'محتوى القالب',
    templateContentHint: 'يُكتب باستخدام حقول الدمج {tokens}. يُستخدم لمعاينة هذا النموذج وطباعته معبأً.',
    fieldsSectionTitle: 'الحقول',
    fieldsHintPrefix:
      'أعطِ رمز حقل دمج من القالب أعلاه قائمة منسدلة أو مجموعة اختيار بدلاً من مربع نص عادي عند تعبئة المشروع لهذا النموذج — طابق "الرمز" هنا مع الاسم المستخدم في القالب (مثال:',
    fieldsHintSuffix: 'في القالب يحتاج إلى "plotArea" كرمز له). أي رمز غير مدرج هنا يحصل فقط على مربع نص عادي.',
    fieldType: {
      text: 'نص',
      dropdown: 'قائمة منسدلة',
      radioButtons: 'أزرار اختيار',
    },
    addField: 'إضافة حقل',
    removeField: 'إزالة الحقل',
    fieldTokenPlaceholder: "الرمز (يطابق {'{'}{'{'}token{'}'}{'}'})",
    fieldLabelPlaceholder: 'تسمية الحقل',
    fieldOptionsPlaceholder: 'خيار واحد في كل سطر',
    sampleFormTitle: 'نموذج مرجعي',
    sampleFormHint: 'ارفع نسخة مرجعية من النموذج الحكومي الفعلي (مثل النسخة الرسمية الفارغة PDF) للتحقق من القالب والحقول أعلاه. لا يتم تحليله — مجرد مرفق.',
    currentlyAttached: 'المرفق حالياً: {name}',
    uploading: 'جارٍ الرفع…',
    uploadSample: 'رفع نموذج مرجعي',
    taggedServicesTitle: 'الخدمات المرتبطة',
    taggedServicesHint: 'يُقترح هذا النموذج ضمن قسم مستندات > الجهات الحكومية للمشروع عندما تطابق خدمة المشروع إحدى الوسوم أدناه.',
    noServicesTitle: 'لا توجد خدمات في الكتالوج بعد',
    noServicesDescription: 'أضف خدمات ضمن الإدارة > الكتالوجات > كتالوج الخدمات لوسم هذا النموذج.',
    saveForm: 'حفظ النموذج',
    sampleUploadedTitle: 'تم رفع ملف العينة',
    sampleUploadedDescription: 'تم إرفاق {name} بهذا النموذج.',
  },

  loadStandardFormsDialog: {
    title: 'تحميل النماذج القياسية',
    description:
      'يضيف أياً من اتفاقيات وتعهدات التصميم/الترخيص القياسية للمكتب إلى مكتبة النماذج. يُنشأ كل منها كنموذج قابل للتعديل — عدّل قالبه وخدماته المرتبطة وحالته لاحقاً كأي نموذج آخر.',
    noAuthorityTitle: 'أضف جهة أولاً',
    noAuthorityDescription:
      'يجب حفظ هذه النماذج تحت جهة (مثل جهة "داخلية" للمكتب الهندسي نفسه) قبل استيرادها.',
    fileUnderAuthority: 'الحفظ تحت الجهة',
    standardFormsCount: 'النماذج القياسية ({count})',
    alreadyAdded: 'مُضاف مسبقاً',
    addForms: 'إضافة نموذج | إضافة {count} نموذج | إضافة {count} نماذج',
  },

  templateFieldMapperDialog: {
    mapFieldsTitle: 'ربط الحقول — {filename}',
    intro:
      'انقر على سطر في المستند أدناه، ثم انقر على حقل لإدراجه في ذلك الموضع — سيظهر كوسم صغير، وليس كرمز. لجدول متكرر (مثل بنود القائمة)، انقر على الصف الذي يجب أن يتكرر، ثم انقر على كل عمود ليوضع في خليته.',
    tableColumnsHint: 'أعمدة {label} — انقر على خلية جدول أولاً:',
    listFieldHint: '{label} — انقر على السطر الذي يجب أن يتكرر أولاً:',
    repeatingAt: 'يتكرر عند {label}',
    repeatsForEach: 'يتكرر لكل: {label}',
    repeats: 'يتكرر: {label}',
    clearRepeatingField: 'إزالة الحقل المتكرر',
    searchPlaceholder: 'ابحث عن نص في المستند...',
    emptyLine: '(سطر فارغ)',
    saveMapping: 'حفظ الربط',
    clickIntoDocumentFirstTitle: 'انقر داخل المستند أولاً',
    clickIntoDocumentFirstDescription: 'انقر على سطر أو خلية جدول أدناه، ثم انقر على حقل لإدراجه هناك.',
    fieldsMappedTitle: 'تم ربط الحقول',
    fieldsMappedDescription: 'تم تحديث {filename}.',
    couldNotSaveMapping: 'تعذّر حفظ الربط',
  },

  userManagementPage: {
    pageTitle: 'إدارة المستخدمين',
    pageSubtitle: 'إدارة المستخدمين والأدوار والصلاحيات على مستوى الشركة.',
    addUser: 'إضافة مستخدم',
    usersTab: 'المستخدمون',
    rolesTab: 'الأدوار والصلاحيات',
    backToUsers: 'العودة إلى المستخدمين',
    userNotFoundTitle: 'المستخدم غير موجود',
    userNotFoundDescription: 'هذا المستخدم غير موجود، أو ربما تم حذفه.',
    allRoles: 'جميع الأدوار',
    noUsersFound: 'لا يوجد مستخدمون',
    noUsersFoundDescription: 'حاول تعديل البحث أو عوامل التصفية، أو أضف مستخدمًا جديدًا.',
    columnName: 'الاسم',
    columnDesignation: 'المسمى الوظيفي',
    columnEmail: 'البريد الإلكتروني',
    columnRole: 'الدور',
    columnStatus: 'الحالة',
    userProfile: 'الملف الشخصي للمستخدم',
    deactivate: 'إلغاء التفعيل',
    activate: 'تفعيل',
    editUser: 'تعديل المستخدم',
    resetPassword: 'إعادة تعيين كلمة المرور',
    deleteUser: 'حذف المستخدم',
    userUpdatedTitle: 'تم تحديث المستخدم',
    userUpdatedDescription: 'تم تحديث {name} بنجاح.',
    userAddedTitle: 'تمت إضافة المستخدم',
    userAddedDescription: 'تمت إضافة {name} إلى المكتب.',
    loginCreatedFor: 'تم إنشاء تسجيل دخول لـ {name}',
    failedToUpdateUser: 'فشل تحديث المستخدم',
    failedToAddUser: 'فشل إضافة المستخدم',
    userActivatedTitle: 'تم تفعيل المستخدم',
    userDeactivatedTitle: 'تم إلغاء تفعيل المستخدم',
    userIsNowActive: 'أصبح {name} نشطًا الآن.',
    userIsNowInactive: 'أصبح {name} غير نشط الآن.',
    passwordResetFailed: 'فشلت إعادة تعيين كلمة المرور',
    userDeletedTitle: 'تم حذف المستخدم',
    userDeletedDescription: 'تمت إزالة {name} من المكتب.',
    failedToDeleteUser: 'فشل حذف المستخدم',
    resetPasswordConfirmMessage: 'إنشاء كلمة مرور عشوائية جديدة لـ {name}؟ ستتوقف كلمة مروره الحالية عن العمل فورًا.',
    deleteUserConfirmMessage: 'إزالة {name} نهائيًا من المكتب؟ سيفقد الوصول فورًا ولا يمكن التراجع عن هذا.',
  },

  governmentFormsPanel: {
    addAuthority: 'إضافة جهة',
    loadStandardForms: 'تحميل النماذج القياسية',
    addForm: 'إضافة نموذج',
    backToForms: 'العودة إلى النماذج',
    formNotFoundTitle: 'النموذج غير موجود',
    formNotFoundDescription: 'هذا النموذج غير موجود، أو ربما تم حذفه.',
    authority: 'الجهة',
    editAuthority: 'تعديل الجهة',
    deleteAuthority: 'حذف الجهة',
    status: 'الحالة',
    noAuthoritiesYet: 'لا توجد جهات بعد — أضف واحدة للبدء بتقديم النماذج تحتها.',
    searchPlaceholder: 'ابحث عن النماذج بالعنوان أو الرمز',
    noFormsFound: 'لا توجد نماذج',
    noFormsFoundDescription: 'أضف نموذجًا حكوميًا لبناء المكتبة.',
    columnCode: 'الرمز',
    columnFormTitle: 'عنوان النموذج',
    columnCategory: 'الفئة',
    columnLanguage: 'اللغة',
    columnVersion: 'الإصدار',
    columnStatus: 'الحالة',
    columnLastUpdated: 'آخر تحديث',
    previewForm: 'معاينة النموذج',
    disableForm: 'تعطيل النموذج',
    enableForm: 'تفعيل النموذج',
    editForm: 'تعديل النموذج',
    deleteForm: 'حذف النموذج',
    allAuthoritiesCount: 'جميع الجهات ({count})',
    authorityWithCount: '{name} ({count})',
    authorityUpdatedTitle: 'تم تحديث الجهة',
    authorityUpdatedDescription: 'تم حفظ {name}.',
    authorityAddedTitle: 'تمت إضافة الجهة',
    authorityAddedDescription: 'أصبح {name} متاحًا للنماذج الآن.',
    unableToSaveAuthority: 'تعذّر حفظ الجهة',
    formUpdatedTitle: 'تم تحديث النموذج',
    formUpdatedDescription: 'تم حفظ {title}.',
    formAddedTitle: 'تمت إضافة النموذج',
    formAddedDescription: 'تمت إضافة {title} إلى المكتبة.',
    unableToSaveForm: 'تعذّر حفظ النموذج',
    authorityRemovedTitle: 'تمت إزالة الجهة',
    authorityRemovedDescription: 'تمت إزالة {label} ونماذجه.',
    formRemovedTitle: 'تمت إزالة النموذج',
    formRemovedDescription: 'تمت إزالة {label} من المكتبة.',
    unableToDelete: 'تعذّر الحذف',
    formDisabledTitle: 'تم تعطيل النموذج',
    formDisabledDescription: 'أصبح {title} مؤرشفًا ومخفيًا عن المشاريع الآن.',
    formEnabledTitle: 'تم تفعيل النموذج',
    formEnabledDescription: 'أصبح {title} نشطًا مجددًا.',
    unableToUpdateStatus: 'تعذّر تحديث الحالة',
    standardFormsAddedTitle: 'تمت إضافة النماذج القياسية',
    standardFormsAddedDescription: 'تمت إضافة {count} نماذج إلى المكتبة.',
    standardFormsAddedDescriptionOne: 'تمت إضافة نموذج واحد إلى المكتبة.',
    unableToImportForms: 'تعذّر استيراد النماذج',
    unableToImportFormsDescription: 'قد لا تكون بعض النماذج قد أُضيفت. يرجى المحاولة مرة أخرى.',
    confirmDeletionTitle: 'تأكيد الحذف',
    confirmDeletionMessage: 'هل أنت متأكد من حذف "{label}"؟ لا يمكن التراجع عن هذا.',
  },

  permissionMatrix: {
    module: 'الوحدة',
    view: 'عرض',
    edit: 'تعديل',
    delete: 'حذف',
    noPermissionsDefined: 'لا توجد صلاحيات معرّفة',
  },

  roleCard: {
    userCount: '{count} مستخدم',
    usersCount: '{count} مستخدمين',
    editPermissions: 'تعديل الصلاحيات',
    saveChanges: 'حفظ التغييرات',
    permissionsUpdatedTitle: 'تم تحديث الصلاحيات',
    permissionsUpdatedDescription: 'تم حفظ صلاحيات {role}.',
    updateFailed: 'فشل التحديث',
    updateRolePermissionsTitle: 'تحديث صلاحيات الدور',
    updateRolePermissionsMessage: 'هذا يغيّر الوصول لكل مستخدم مُسند إليه دور {role}. متابعة؟',
  },

  permitCatalog: {
    permitName: 'اسم التصريح',
    fixedCost: 'التكلفة الثابتة',
    invalidCost: 'يجب أن تكون التكلفة صفراً أو رقماً موجباً.',
    removePermit: 'إزالة التصريح',
    noPermitsYet: 'لا توجد تصاريح بعد — أضف واحدًا أدناه.',
    addPermit: 'إضافة تصريح',
    add: 'إضافة',
    prerequisites: 'المتطلبات المسبقة',
    prerequisitesDescription: 'يصبح هذا التصريح جاهزًا للتقديم على مشروع فقط بعد اكتمال كل نشاط تصميم مذكور هنا.',
    noPrerequisitesYet: 'لا توجد متطلبات مسبقة — هذا التصريح جاهز فور اختياره في مشروع.',
    addPrerequisitePlaceholder: 'اختر نشاط تصميم',
    removePrerequisite: 'إزالة المتطلب المسبق',
    failedToLoadPrerequisites: 'فشل تحميل المتطلبات المسبقة',
    failedToAddPrerequisite: 'فشلت إضافة المتطلب المسبق',
    failedToRemovePrerequisite: 'فشلت إزالة المتطلب المسبق',
    permitSettings: 'إعدادات التصريح',
    setupMissing: 'غير مُعدّ',
    applicationSetupDescription: 'تبدأ الطلبات لهذا النوع من التصاريح بالجهة والنموذج وقائمة المستندات المحددة هنا — لا يحتاج الموظفون إلى اختيار شيء.',
    authority: 'الجهة',
    selectAuthority: 'اختر جهة',
    form: 'نموذج الطلب',
    selectForm: 'اختر نموذجًا',
    requiredDocuments: 'المستندات المطلوبة',
    requiredDocumentsHint: 'مستند واحد في كل سطر. اتركه فارغًا لاستخدام قائمة النموذج نفسه (تظهر كنص توضيحي).',
    saveSetup: 'حفظ الإعداد',
    setupSaved: 'تم حفظ إعداد الطلب',
  },

  serviceCatalog: {
    activitiesCount: '{count} نشاط',
    activityCountOne: 'نشاط واحد',
    branchDesign: 'تصميم',
    branchSupervision: 'إشراف',
    branchDesignOneTime: 'تصميم (لمرة واحدة)',
    branchSupervisionMonthly: 'إشراف (شهري)',
    monthlyNote: '(شهري)',
    activityName: 'اسم النشاط',
    fixedCost: 'التكلفة الثابتة',
    invalidCost: 'يجب أن تكون التكلفة صفراً أو رقماً موجباً.',
    removeActivity: 'إزالة النشاط',
    noActivitiesYet: 'لا توجد أنشطة بعد — أضف واحدًا أدناه.',
    prerequisites: 'المتطلبات المسبقة',
    prerequisitesDescription: 'يصبح نشاط الإشراف هذا جاهزًا للبدء في مشروع فقط بعد اكتمال كل نشاط تصميم مذكور هنا.',
    noPrerequisitesYet: 'لا توجد متطلبات مسبقة — هذا النشاط جاهز فور اختياره في مشروع.',
    addPrerequisitePlaceholder: 'اختر نشاط تصميم',
    removePrerequisite: 'إزالة المتطلب المسبق',
    failedToLoadPrerequisites: 'فشل تحميل المتطلبات المسبقة',
    failedToAddPrerequisite: 'فشلت إضافة المتطلب المسبق',
    failedToRemovePrerequisite: 'فشلت إزالة المتطلب المسبق',
    addActivity: 'إضافة نشاط',
    add: 'إضافة',
    addService: 'إضافة خدمة',
    serviceName: 'اسم الخدمة',
    selectService: 'اختر خدمة',
    selectServiceDescription: 'اختر خدمة من القائمة على اليسار، أو أضف خدمة جديدة، لعرض أنشطتها وتعديلها.',
    removeService: 'إزالة الخدمة',
  },

  mergeField: {
    remove: 'إزالة {label}',
  },

  documentTemplates: {
    backToDocuments: 'العودة إلى المستندات',
    templateNotFoundTitle: 'القالب غير موجود',
    templateNotFoundDescription: 'هذا القالب غير موجود، أو ربما تم حذفه.',
    quotationTitle: 'قوالب عروض الأسعار',
    quotationDescription: 'يُدمج ملف .docx الافتراضي لكل لغة مع بيانات المشروع الحية عند استخدام "تنزيل المستند" على عرض سعر.',
    contractTitle: 'قوالب العقود',
    contractDescription: 'يُدمج ملف .docx الافتراضي لكل لغة مع بيانات المشروع الحية عند استخدام "تنزيل المستند" على عقد.',
    paymentPlanTitle: 'قوالب خطة الدفع',
    paymentPlanDescription: 'يُدمج ملف .docx الافتراضي لكل لغة مع بيانات خطة الدفع الحية للمشروع (كل مسار دفع له اتفاقية) عند استخدام "تنزيل المستند" في صفحة خطة الدفع.',
    quotation: 'عرض السعر',
    contract: 'العقد',
    paymentPlan: 'خطة الدفع',
    uploadDocx: 'رفع ملف .docx',
    noTemplatesForType: 'لم يتم رفع قوالب {type} بعد.',
    noLanguageTemplates: 'لم يتم رفع قالب {type} بـ{language} بعد.',
    default: 'افتراضي',
    setDefault: 'تعيين كافتراضي',
    mapFields: 'ربط الحقول',
    downloadTemplate: 'تنزيل القالب',
    deleteTemplate: 'حذف القالب',
    templateUploadedTitle: 'تم رفع القالب',
    templateUploadedDescription: 'تم رفع {filename}.',
    defaultUpdatedTitle: 'تم تحديث القالب الافتراضي',
    defaultUpdatedDescription: 'أصبح {filename} هو قالب {type} الافتراضي الآن.',
    couldNotSetDefault: 'تعذّر التعيين كافتراضي',
    templateDeletedTitle: 'تم حذف القالب',
    templateDeletedDescription: 'تم حذف {filename}.',
    deleteFailed: 'فشل الحذف',
    deleteTemplateConfirmTitle: 'حذف القالب',
    deleteTemplateConfirmMessage: 'حذف "{filename}"؟ لا يمكن التراجع عن هذا.',
    layout: 'الترويسة والتخطيط',
    layoutDialogTitle: 'ترويسة الصفحة وتخطيطها',
    letterhead: 'خلفية الترويسة',
    letterheadHint:
      'صورة PNG أو JPEG من القرطاسية المطبوعة (الشعار، الترويسة، التذييل). تُدمج خلف كل صفحة بالكامل بحيث يبدو المستند وكأنه طُبع مباشرة على هذه الورقة.',
    letterheadUploadHint: 'PNG أو JPEG',
    removeBackground: 'إزالة الخلفية',
    backgroundUploadedTitle: 'تم تحديث الخلفية',
    backgroundUploadedDescription: 'تم رفع خلفية الترويسة.',
    orientation: 'الاتجاه',
    portrait: 'عمودي',
    landscape: 'أفقي',
    margins: 'الهوامش (مم)',
    marginTop: 'أعلى',
    marginRight: 'يمين',
    marginBottom: 'أسفل',
    marginLeft: 'يسار',
    layoutSavedTitle: 'تم حفظ التخطيط',
    layoutSavedDescription: 'تم تحديث الاتجاه والهوامش.',
  },

  serviceDocumentMap: {
    noServicesYet: 'لا توجد خدمات في الكتالوج بعد',
    noServicesYetDescription: 'أضف خدمات ضمن الإدارة > الكتالوجات > كتالوج الخدمات أولاً.',
    noFillableFormsYet: 'لا توجد نماذج قابلة للتعبئة بعد',
    noFillableFormsYetDescription:
      'أضف محتوى قالب لنموذج ضمن الإدارة > المستندات > النماذج الحكومية لجعله قابلاً للتخصيص هنا.',
    failedToUpdate: 'فشل التحديث',
  },

  scheduledReportsPage: {
    pageTitle: 'التقارير المجدولة',
    pageSubtitle: 'إرسال تقرير PDF تلقائيًا عبر البريد الإلكتروني إلى شخص واحد أو أكثر، مرة واحدة أو وفق جدول متكرر.',
    addSchedule: 'جدولة جديدة',
    backToSchedules: 'العودة إلى التقارير المجدولة',
    scheduleNotFoundTitle: 'الجدولة غير موجودة',
    scheduleNotFoundDescription: 'ربما تمت إزالة هذا التقرير المجدول أو أن الرابط غير صحيح.',
    columnName: 'الاسم',
    columnReport: 'التقرير',
    columnSchedule: 'الجدول',
    columnRecipients: 'المستلمون',
    columnStatus: 'الحالة',
    columnLastRun: 'آخر تشغيل',
    noSchedulesFound: 'لا توجد تقارير مجدولة بعد',
    noSchedulesFoundDescription: 'أنشئ واحدًا لإرسال تقرير PDF تلقائيًا عبر البريد الإلكتروني وفق جدول.',
    active: 'نشط',
    paused: 'متوقف مؤقتًا',
    lastRunSent: 'تم الإرسال',
    lastRunFailed: 'فشل',
    neverRun: 'لم يتم التشغيل بعد',
    nextRun: 'التالي: {when}',
    noNextRun: 'لا يوجد تشغيل قادم',

    reportTypeBusinessSummary: 'ملخص الأعمال',
    reportTypeFinancialSummary: 'الملخص المالي',
    reportTypeProjectStatus: 'حالة المشروع',

    periodLast7Days: 'آخر 7 أيام',
    periodLast30Days: 'آخر 30 يومًا',
    periodThisMonth: 'هذا الشهر',
    periodLastMonth: 'الشهر الماضي',
    periodThisQuarter: 'هذا الربع',
    periodThisYear: 'هذا العام',

    frequencyOnce: 'مرة واحدة',
    frequencyDaily: 'يوميًا',
    frequencyWeekly: 'أسبوعيًا',
    frequencyMonthly: 'شهريًا',

    weekdayMonday: 'الإثنين',
    weekdayTuesday: 'الثلاثاء',
    weekdayWednesday: 'الأربعاء',
    weekdayThursday: 'الخميس',
    weekdayFriday: 'الجمعة',
    weekdaySaturday: 'السبت',
    weekdaySunday: 'الأحد',

    createDialogTitle: 'تقرير مجدول جديد',
    editDialogTitle: 'تعديل التقرير المجدول',
    sectionBasics: 'التقرير',
    name: 'اسم الجدولة',
    namePlaceholder: 'مثال: الملخص المالي الأسبوعي للإدارة',
    nameRequired: 'اسم الجدولة مطلوب.',
    reportType: 'التقرير',
    project: 'المشروع',
    projectPlaceholder: 'اختر مشروعًا',
    period: 'الفترة',

    sectionRecipients: 'المستلمون',
    recipientsHint: 'عنوان بريد إلكتروني واحد في كل سطر، أو مفصولة بفواصل.',
    recipientsPlaceholder: "name{'@'}company.com",
    subject: 'عنوان البريد الإلكتروني',
    subjectPlaceholder: 'اتركه فارغًا لاستخدام عنوان التقرير',
    messageBody: 'الرسالة',
    messageBodyPlaceholder: 'اتركه فارغًا للرسالة الافتراضية',

    sectionSchedule: 'الجدول',
    frequency: 'التكرار',
    sendDate: 'تاريخ الإرسال',
    sendTimeLabel: 'وقت الإرسال',
    sendTimeRequired: 'وقت الإرسال مطلوب.',
    dayOfWeek: 'يوم الأسبوع',
    dayOfMonth: 'يوم الشهر',
    startDate: 'تاريخ البدء',
    startDateRequired: 'تاريخ البدء مطلوب.',
    endDate: 'تاريخ الانتهاء',
    endDateHint: 'اتركه فارغًا لعدم وجود تاريخ انتهاء -- يستمر الجدول في العمل إلى أجل غير مسمى.',
    isActive: 'نشط',
    isActiveHint: 'أوقفه لإيقاف هذه الجدولة مؤقتًا دون حذفها.',

    save: 'حفظ الجدولة',
    sendTestNow: 'إرسال تجريبي الآن',
    testSentTitle: 'تم إرسال البريد التجريبي',
    testSentDescription: 'تم للتو إرسال نسخة تجريبية من \'{name}\' إلى مستلميها.',
    testFailedTitle: 'فشل الإرسال التجريبي',
    scheduleSavedTitle: 'تم حفظ التقرير المجدول',
    scheduleSavedDescription: 'تم حفظ \'{name}\'.',
    saveFailedTitle: 'تعذر الحفظ',
    deleteSchedule: 'حذف الجدولة',
    deleteScheduleConfirmMessage: 'حذف التقرير المجدول \'{name}\'؟ لا يمكن التراجع عن هذا الإجراء.',
    scheduleDeletedTitle: 'تم حذف التقرير المجدول',
    scheduleDeletedDescription: 'تم حذف \'{name}\'.',
    deleteFailedTitle: 'تعذر الحذف',
  },
}
