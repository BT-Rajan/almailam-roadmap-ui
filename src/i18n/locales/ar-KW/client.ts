export default {
  unassigned: 'غير معيّن',
  unknownClient: 'عميل غير معروف',

  card: {
    openClient: 'فتح ملف العميل {name}',
  },

  header: {
    deactivate: 'إلغاء التفعيل',
    reactivate: 'إعادة التفعيل',
    editClient: 'تعديل العميل',
    deleteClient: 'حذف العميل',
  },

  addressCard: {
    edit: 'تعديل عنوان {type}',
    remove: 'إزالة عنوان {type}',
  },

  contactList: {
    title: 'جهات الاتصال',
    emptyTitle: 'لا توجد جهات اتصال مسجلة',
    emptyDescription: 'أضف جهة اتصال رئيسية حتى يمكن التواصل مع هذا العميل أثناء التهيئة.',
    authorisedRepresentative: '(ممثل مفوض)',
    edit: 'تعديل جهة الاتصال {name}',
    remove: 'إزالة جهة الاتصال {name}',
  },

  documentCard: {
    recordVerification: 'تسجيل التحقق',
    download: 'تنزيل المستند',
    versionHistory: 'سجل الإصدارات',
    replaceFile: 'استبدال الملف',
    edit: 'تعديل المستند',
    remove: 'إزالة المستند',
  },

  identificationList: {
    title: 'الهوية',
    emptyTitle: 'لا توجد وثائق هوية مسجلة',
    emptyDescription: 'ستظهر هنا وثائق الهوية أو التراخيص المسجلة لهذا العميل.',
    expired: 'منتهية الصلاحية',
    viewDocument: 'عرض المستند المرفوع',
    edit: 'تعديل بيانات الهوية',
    remove: 'إزالة بيانات الهوية',
    summary: 'صدرت في {issueDate} · تنتهي في {expiryDate} · {country}',
  },

  onboardingActions: {
    title: 'حالة التهيئة',
    currentStatus: 'الحالة الحالية',
    recommendedPrefix: 'استناداً إلى المستندات والتحققات المسجلة، يمكن نقل هذا العميل إلى',
    noFurtherChanges: 'لا تتوفر تغييرات أخرى للحالة من هذه المرحلة.',
    advance: 'تقديم',
    changeStatus: 'تغيير الحالة',
    verifyEmail: 'التحقق من العميل',
  },

  verificationList: {
    title: 'سجلات التحقق',
    emptyTitle: 'لا توجد سجلات تحقق',
    emptyDescription: 'ستظهر هنا عمليات التحقق المسجلة لهذا العميل.',
  },

  workspaceTabs: {
    sectionsAria: 'أقسام مساحة عمل العميل',
    overview: 'نظرة عامة',
    contacts: 'جهات الاتصال',
    identification: 'الهوية',
    documents: 'المستندات',
    projects: 'المشاريع',
    details: 'التفاصيل',
  },

  // نظرة عامة على مساحة عمل العميل: أين وصل كل مشروع.
  projectStages: {
    currentlyAt: 'المرحلة الحالية:',
    target: 'الموعد المستهدف {date}',
    openProject: 'فتح المشروع',
  },

  duplicateAlert: {
    title: 'تم العثور على عميل مطابق محتمل',
    description: 'قد يكون هذا العميل موجوداً بالفعل في النظام. راجع التطابقات أدناه قبل إنشاء ملف جديد.',
    matchedOn: 'تطابق في {fields} · {code}',
    viewProfile: 'عرض الملف',
    viewProfileFor: 'عرض ملف {name}',
  },

  workspacePage: {
    duplicateTitle: 'تم العثور على عميل مكرر محتمل',
    duplicateDescription: 'يشترك هذا العميل في نفس {fields} مع {name} ({code}). إذا كان هذا فعلاً نفس الشخص أو الجهة، فادمج السجلين.',
    review: 'مراجعة',
    notFoundTitle: 'العميل غير موجود',
    notFoundDescription: 'ربما تمت إزالة هذا العميل أو أن الرابط غير صحيح.',
    profileInformation: 'معلومات الملف',
    contactDetails: 'بيانات الاتصال',
    internalNotes: 'ملاحظات داخلية',
    noInternalNotes: 'لم تتم إضافة أي ملاحظات داخلية بعد.',
    messageClient: 'مراسلة العميل',
    addresses: 'العناوين',
    addAddress: 'إضافة عنوان',
    noAddressTitle: 'لا يوجد عنوان مسجل',
    noAddressDescription: 'أضف عنواناً لإكمال ملف هذا العميل.',
    addContact: 'إضافة جهة اتصال',
    addIdentification: 'إضافة هوية',
    addDocument: 'إضافة مستند',
    noDocumentsTitle: 'لا توجد مستندات مسجلة',
    noDocumentsDescription: 'ارفع وثائق الهوية أو التسجيل أو التفويض الخاصة بهذا العميل.',
    newProject: 'مشروع جديد',
    clientMustBeActive: 'يجب أن يكون العميل نشطاً قبل إنشاء مشروع.',
    viewIdentificationDocuments: 'عرض مستندات الهوية',
    noProjectsTitle: 'لا توجد مشاريع بعد',
    noProjectsDescription: 'ستظهر هنا المشاريع التي يتم إنشاؤها لهذا العميل.',
    fields: {
      fullLegalName: 'الاسم القانوني الكامل',
      preferredName: 'الاسم المفضل',
      nationality: 'الجنسية',
      dateOfBirth: 'تاريخ الميلاد',
      countryOfResidence: 'بلد الإقامة',
      legalName: 'الاسم القانوني',
      tradeName: 'الاسم التجاري',
      registrationNumber: 'رقم السجل',
      tradeLicenceNumber: 'رقم الرخصة التجارية',
      countryOfRegistration: 'بلد التسجيل',
      contactPerson: 'جهة الاتصال',
      mobile: 'الجوال',
      email: 'البريد الإلكتروني',
      city: 'المدينة',
      preferredChannel: 'قناة التواصل المفضلة',
      accountManager: 'مدير الحساب',
    },
    resultDialog: {
      documentAddedTitle: 'تمت إضافة المستند',
      uploadedSuccessfully: 'تم رفع {title} بنجاح.',
      failedToUploadDocument: 'فشل رفع المستند',
      failedToDownloadDocument: 'فشل تنزيل المستند',
      failedToOpenDocument: 'فشل فتح المستند',
      fileReplacedTitle: 'تم استبدال الملف',
      updatedToNewVersion: 'تم تحديث {title} إلى نسخة جديدة.',
      failedToReplaceFile: 'فشل استبدال الملف',
      failedToLoadVersionHistory: 'فشل تحميل سجل الإصدارات',
      failedToDownloadVersion: 'فشل تنزيل الإصدار',
      onboardingStatusUpdatedTitle: 'تم تحديث حالة التهيئة',
      statusChangedTo: 'تم تغيير الحالة إلى "{status}".',
      failedToUpdateOnboardingStatus: 'فشل تحديث حالة التهيئة',
      needsManualDecision: 'هذا العميل في حالة تتطلب قراراً يدوياً بالفعل — استخدم تغيير الحالة.',
      statusAdvancedFrom: 'تم تقديم الحالة من "{before}" إلى "{after}".',
      failedToAdvanceOnboardingStatus: 'فشل تقديم حالة التهيئة',
      verificationRecordedTitle: 'تم تسجيل التحقق',
      markedAsResult: 'تم وضع علامة على "{item}" بـ {result}.',
      failedToRecordVerification: 'فشل تسجيل التحقق',
      clientVerifiedTitle: 'تم التحقق من العميل',
      welcomeEmailSent: 'تم تأكيد البريد الإلكتروني للعميل. تم إرسال رسالة ترحيب تحتوي على بيانات دخول بوابة العميل إلى {email}.',
      clientUpdatedTitle: 'تم تحديث العميل',
      changesSavedSuccessfully: 'تم حفظ التغييرات بنجاح.',
      failedToUpdateClient: 'فشل تحديث العميل',
      statusUpdatedTitle: 'تم تحديث الحالة',
      clientMarkedAs: 'تم وضع علامة على العميل بـ {status}.',
      failedToUpdateStatus: 'فشل تحديث الحالة',
      clientDeletedTitle: 'تم حذف العميل',
      failedToDeleteClient: 'فشل حذف العميل',
      clientsMergedTitle: 'تم دمج العملاء',
      mergedInto: 'تم الدمج ضمن {name}.',
      failedToMergeClients: 'فشل دمج العملاء',
      contactUpdatedTitle: 'تم تحديث جهة الاتصال',
      contactAddedTitle: 'تمت إضافة جهة الاتصال',
      failedToUpdateContact: 'فشل تحديث جهة الاتصال',
      failedToAddContact: 'فشل إضافة جهة الاتصال',
      addressUpdatedTitle: 'تم تحديث العنوان',
      addressUpdatedDescription: 'تم تحديث العنوان.',
      addressAddedTitle: 'تمت إضافة العنوان',
      addressAddedDescription: 'تمت إضافة العنوان.',
      failedToUpdateAddress: 'فشل تحديث العنوان',
      failedToAddAddress: 'فشل إضافة العنوان',
      identificationUpdatedTitle: 'تم تحديث الهوية',
      identificationAddedTitle: 'تمت إضافة الهوية',
      failedToUpdateIdentification: 'فشل تحديث الهوية',
      failedToAddIdentification: 'فشل إضافة الهوية',
      documentUpdatedTitle: 'تم تحديث المستند',
      failedToUpdateDocument: 'فشل تحديث المستند',
      removedTitle: 'تمت الإزالة',
      failedToRemove: 'فشلت الإزالة',
      wasAdded: 'تمت إضافة {name}.',
      wasUpdated: 'تم تحديث {name}.',
      wasRemoved: 'تمت إزالة {name}.',
    },
    deleteRecordDialog: {
      title: 'إزالة السجل',
      message: 'إزالة {label}؟ لا يمكن التراجع عن هذا من داخل التطبيق.',
      confirmLabel: 'إزالة',
    },
    deleteClientDialog: {
      message: 'حذف {name}؟ لا يمكن التراجع عن هذا من داخل التطبيق، ويُحظر إذا كان لدى العميل أي مشاريع مسجلة.',
    },
  },

  basicInfoStep: {
    clientTypeTitle: 'نوع العميل',
    personalInformation: 'المعلومات الشخصية',
    organisationInformation: 'معلومات الجهة',
    contactDetails: 'بيانات الاتصال',
    assignment: 'التعيين',
    assignmentDescription: 'من في الفريق مسؤول عن علاقة هذا العميل. يمكن تغييره لاحقاً.',
    fullLegalName: 'الاسم القانوني الكامل',
    preferredName: 'الاسم المفضل',
    nationality: 'الجنسية',
    dateOfBirth: 'تاريخ الميلاد',
    legalName: 'الاسم القانوني',
    tradeName: 'الاسم التجاري',
    organisationType: 'نوع الجهة',
    registrationNumber: 'رقم السجل',
    tradeLicenceNumber: 'رقم الرخصة التجارية',
    countryOfRegistration: 'بلد التسجيل',
    dateOfIncorporation: 'تاريخ التأسيس',
    website: 'الموقع الإلكتروني',
    mobileNumber: 'رقم الجوال',
    emailAddress: 'البريد الإلكتروني',
    preferredLanguage: 'اللغة المفضلة',
    accountManager: 'مدير الحساب',
    accountManagerLoadFailed: 'تعذر تحميل قائمة مديري الحسابات.',
    retry: 'إعادة المحاولة',
  },

  contactAddressStep: {
    contactsTitle: 'جهات الاتصال',
    contactsDescription: 'أضف جهة اتصال واحدة على الأقل لهذا العميل.',
    contactNumber: 'جهة الاتصال {number}',
    removeContact: 'إزالة جهة الاتصال {number}',
    name: 'الاسم',
    contactType: 'نوع جهة الاتصال',
    mobileNumber: 'رقم الجوال',
    emailAddress: 'البريد الإلكتروني',
    authorisedToggle: 'مفوض للتصرف نيابة عن العميل',
    addContact: 'إضافة جهة اتصال',
    addressTitle: 'العنوان',
    addressDescription: 'أضف عنوان العميل.',
    addressDescriptionOrganisation: 'أضف العنوان المسجل لهذا العميل',
    addressType: 'نوع العنوان',
    country: 'الدولة',
    state: 'المحافظة',
    city: 'المدينة',
    area: 'المنطقة',
    street: 'الشارع',
    building: 'المبنى',
  },

  identificationStep: {
    title: 'الهوية',
    entityDescription: 'سجّل الرخصة التجارية لهذا العميل، بما في ذلك تاريخي الإصدار والانتهاء.',
    individualDescription: 'سجّل وثيقة الهوية الأساسية للعميل، بما في ذلك تاريخي الإصدار والانتهاء.',
    documentType: 'نوع المستند',
    documentNumber: 'رقم المستند',
    registrationNumber: 'رقم التسجيل',
    issueDate: 'تاريخ الإصدار',
    expiryDate: 'تاريخ الانتهاء',
    issuingCountry: 'بلد الإصدار',
    uploadTitle: 'رفع المستند',
    uploadDescription: 'ارفع نسخة من وثيقة الهوية أو الرخصة التجارية. بصيغة JPG أو PNG أو PDF، بحد أقصى 5 ميجابايت.',
  },

  reviewStep: {
    summaryTitle: 'ملخص العميل',
    clientType: 'نوع العميل',
    name: 'الاسم',
    mobile: 'الجوال',
    email: 'البريد الإلكتروني',
    city: 'المدينة',
    contactsTitle: 'جهات الاتصال',
    contactNumber: 'جهة الاتصال {number}',
    identificationTitle: 'الهوية',
    documentType: 'نوع المستند',
    documentNumber: 'رقم المستند',
    documentUploaded: 'تم رفع المستند',
    notYetUploaded: 'لم يُرفع بعد',
  },

  clientsPage: {
    title: 'العملاء',
    subtitle: 'تهيئة والتحقق من ملفات العملاء الأفراد والجهات وإعادة استخدامها عبر جميع المشاريع.',
    newClient: 'عميل جديد',
    clientType: 'نوع العميل',
    allTypes: 'جميع الأنواع',
    status: 'الحالة',
    myClients: 'عملائي',
    layoutAria: 'تخطيط قائمة العملاء',
    gridView: 'عرض شبكي',
    tableView: 'عرض جدولي',
    noClientsAssignedTitle: 'لا يوجد عملاء معيّنون لك بعد',
    noClientsFoundTitle: 'لا يوجد عملاء',
    noClientsAssignedDescription: 'لا يوجد لديك عملاء معيّنون كمدير حساب لهم. أوقف تفعيل "عملائي" لرؤية الجميع، أو عيّن نفسك عبر تعديل العميل.',
    noClientsAssignedDescriptionTable: 'لا يوجد لديك عملاء معيّنون كمدير حساب لهم. أوقف تفعيل "عملائي" لرؤية الجميع.',
    noClientsFoundDescription: 'حاول تعديل عوامل التصفية، أو أضف عميلاً جديداً.',
    columns: {
      code: 'الرمز',
      name: 'الاسم',
      type: 'النوع',
      mobile: 'الجوال',
      email: 'البريد الإلكتروني',
      city: 'المدينة',
      accountManager: 'مدير الحساب',
      status: 'الحالة',
    },
    deletedClients: 'العملاء المحذوفون',
    backToClients: 'العودة إلى العملاء',
    noDeletedClientsTitle: 'لا يوجد عملاء محذوفون',
    noDeletedClientsDescription: 'سيظهر هنا العملاء الذين تم حذفهم، ويمكن استعادتهم في أي وقت.',
    restore: 'استعادة',
    restoredTitle: 'تمت استعادة العميل',
    restoredDescription: 'تمت استعادة {name}.',
    failedToRestore: 'فشلت استعادة العميل',
  },

  newWizard: {
    title: 'تهيئة عميل جديد',
    subtitle: 'اجمع بيانات العميل وتحقق منها وأكّدها خلال خطوات قليلة.',
    steps: {
      clientType: 'نوع العميل',
      contactsAddress: 'جهات الاتصال والعنوان',
      identification: 'الهوية',
      reviewConfirm: 'المراجعة والتأكيد',
    },
    cancel: 'إلغاء',
    back: 'رجوع',
    next: 'التالي',
    addClient: 'إضافة عميل',
    resumeDraftTitle: 'استئناف مسودة غير محفوظة؟',
    resumeDraftMessage: 'لديك نموذج تهيئة عميل غير مكتمل محفوظ من وقت سابق. استأنف من حيث توقفت، أو ابدأ من جديد.',
    startFresh: 'البدء من جديد',
    resumeDraft: 'استئناف المسودة',
    clientSubmittedTitle: 'تم إرسال العميل',
    clientSubmittedMessage: 'تم إنشاء العميل الجديد م/ {name} بنجاح',
    clientSubmittedMessageOrganisation: 'تم إنشاء عميل المؤسسة م/س {name} بنجاح.',
    clientIdNotice: 'يرجى تدوين رقم العميل {code} للاستخدام مستقبلاً.',
    viewClientWorkspace: 'عرض مساحة عمل العميل',
    failedToOnboardClient: 'فشلت تهيئة العميل',
    pleaseFixHighlightedFields: 'يرجى تصحيح الحقول المميّزة',
    fieldsNeedAttentionUnderStep: 'بعض الحقول ضمن "{step}" تحتاج إلى مراجعة قبل المتابعة.',
    fieldsNeedAttentionBeforeOnboarding: 'بعض الحقول ضمن "{step}" تحتاج إلى مراجعة قبل تهيئة هذا العميل.',
    confirmDialog: {
      title: 'تأكيد تهيئة العميل',
      description: 'ارفع نسخة ممسوحة ضوئياً من موافقة العميل الموقّعة فعلياً لإنشاء العميل.',
      failedToConfirm: 'فشل تأكيد التهيئة',
    },
    submittingNotice: 'جارٍ إرسال التفاصيل -- قد يستغرق هذا بضع ثوانٍ.',
  },

  addressEditDialog: {
    editTitle: 'تعديل العنوان',
    addTitle: 'إضافة عنوان',
    addressType: 'نوع العنوان',
    country: 'الدولة',
    state: 'المحافظة',
    city: 'المدينة',
    area: 'المنطقة',
    street: 'الشارع',
    building: 'المبنى',
    addAddress: 'إضافة عنوان',
  },

  contactEditDialog: {
    editTitle: 'تعديل جهة الاتصال',
    addTitle: 'إضافة جهة اتصال',
    name: 'الاسم',
    contactType: 'نوع جهة الاتصال',
    mobileNumber: 'رقم الجوال',
    emailAddress: 'البريد الإلكتروني',
    authorisedToggle: 'مخوّل للتصرف نيابة عن العميل',
    addContact: 'إضافة جهة اتصال',
  },

  documentEditDialog: {
    title: 'تعديل المستند',
    documentCategory: 'فئة المستند',
    documentTitle: 'عنوان المستند',
    issueDate: 'تاريخ الإصدار',
    expiryDate: 'تاريخ الانتهاء',
    issuingAuthority: 'جهة الإصدار',
  },

  documentUploadDialog: {
    title: 'إضافة مستند عميل',
    documentCategory: 'فئة المستند',
    documentTitle: 'عنوان المستند',
    documentTitlePlaceholder: 'مثال: تجديد الرخصة التجارية',
    fileHint: 'ملفات PDF أو Word أو صور',
    addDocument: 'إضافة مستند',
  },

  documentVersionDialog: {
    title: 'سجل الإصدارات',
    titleWithName: 'سجل الإصدارات — {name}',
    noVersionsTitle: 'لا توجد إصدارات مسجلة',
    noVersionsDescription: 'لا يوجد ملف مسجل لهذا المستند بعد.',
    version: 'الإصدار {version}',
    current: 'الحالي',
    downloadVersion: 'تنزيل الإصدار {version}',
  },

  editDialog: {
    title: 'تعديل العميل',
    contactDetails: 'بيانات الاتصال',
    contactPerson: 'الشخص المسؤول',
    mobileNumber: 'رقم الجوال',
    emailAddress: 'البريد الإلكتروني',
    city: 'المدينة',
    preferredLanguage: 'اللغة المفضلة',
    preferredChannel: 'قناة الاتصال المفضلة',
    relationship: 'العلاقة',
    accountManager: 'مدير الحساب',
    internalNotes: 'ملاحظات داخلية',
    internalNotesHint: 'التفضيلات، إشارات المخاطر، أو تعليمات التعامل — تظهر للموظفين فقط.',
    personalInformation: 'المعلومات الشخصية',
    fullLegalName: 'الاسم القانوني الكامل',
    preferredName: 'الاسم المفضل',
    nationality: 'الجنسية',
    dateOfBirth: 'تاريخ الميلاد',
    countryOfResidence: 'بلد الإقامة',
    organisationInformation: 'معلومات المنشأة',
    legalName: 'الاسم القانوني',
    tradeName: 'الاسم التجاري',
    organisationType: 'نوع المنشأة',
    registrationNumber: 'رقم السجل',
    tradeLicenceNumber: 'رقم الرخصة التجارية',
    countryOfRegistration: 'بلد التسجيل',
    dateOfIncorporation: 'تاريخ التأسيس',
    website: 'الموقع الإلكتروني',
    languageEnglish: 'الإنجليزية',
    languageArabic: 'العربية',
    channelEmail: 'البريد الإلكتروني',
    channelWhatsapp: 'واتساب',
    channelSms: 'رسالة نصية',
    channelPhone: 'الهاتف',
  },

  identificationEditDialog: {
    editTitle: 'تعديل الهوية',
    addTitle: 'إضافة هوية',
    documentType: 'نوع المستند',
    documentNumber: 'رقم المستند',
    issueDate: 'تاريخ الإصدار',
    expiryDate: 'تاريخ الانتهاء',
    issuingCountry: 'بلد الإصدار',
    addIdentification: 'إضافة هوية',
  },

  mergeDialog: {
    title: 'دمج عميل مكرر',
    description:
      'يشترك هذا العميل في نفس {fields} مع عميل آخر مسجل. إذا كانا فعلياً نفس الشخص أو المنشأة، فادمجهما في سجل واحد. تنتقل جهات الاتصال والعناوين والهويات والمستندات والتحققات والمشاريع جميعها إلى السجل الذي تحتفظ به؛ لا يُحذف شيء نهائياً.',
    whichRecordToKeep: 'أي سجل تريد الاحتفاظ به',
    keepThisRecord: 'الاحتفاظ بهذا السجل',
    keepOtherRecord: 'الاحتفاظ بالسجل الآخر',
    createdOn: 'أُنشئ في {date}',
    cannotBeUndone: 'لا يمكن التراجع عن هذا من داخل التطبيق.',
    override: 'تجاوز',
    mergeClients: 'دمج العملاء',
  },

  onboardingStatusDialog: {
    title: 'تغيير حالة التهيئة',
    newStatus: 'الحالة الجديدة',
    reason: 'السبب',
    reasonRequiredHint: 'مطلوب لهذا التغيير في الحالة',
  },

  emailVerification: {
    title: 'التحقق من العميل',
    description: 'ارفع نسخة ممسوحة ضوئياً من موافقة العميل الموقّعة فعلياً للتأكيد وتحديد حالته كـ"جاهز".',
    failedToConfirm: 'فشل تأكيد التحقق',
  },

  verificationDialog: {
    title: 'تسجيل تحقق',
    whatWasChecked: 'ما تم التحقق منه',
    whatWasCheckedPlaceholder: 'مثال: الرخصة التجارية، رقم السجل',
    result: 'النتيجة',
    saveVerification: 'حفظ التحقق',
  },

  projectDocuments: {
    title: 'مستندات المشاريع',
    noProjectsTitle: 'لا توجد مشاريع بعد',
    noProjectsDescription: 'ستظهر هنا مستندات مشاريع هذا العميل بمجرد توفرها.',
    noneAvailableYet: 'لا توجد مستندات متاحة بعد.',
    signedQuotation: 'عرض السعر الموقّع',
    approvedPaymentPlan: 'خطة الدفع المعتمدة',
    signedContract: 'العقد الموقّع',
    handoverDocument: 'مستند التسليم',
  },
}
