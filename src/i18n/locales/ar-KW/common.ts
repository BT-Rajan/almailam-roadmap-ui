export default {
  appName: 'سيرفس أو إس',
  // Registered legal entity name — kept in its registered form rather
  // than a guessed/transliterated Arabic rendering.
  companyName: 'Almailam Engineering Consultants',
  skipToMainContent: 'تخطَّ إلى المحتوى الرئيسي',
  breadcrumbNav: 'مسار التنقل',
  search: 'بحث',
  searchGlobalPlaceholder: 'ابحث في العملاء، المشاريع، المستندات...',
  notifications: 'الإشعارات',
  messageCentre: 'مركز الرسائل',
  knowledgeAssistant: 'المساعد المعرفي',
  activityCalendar: 'تقويم الأنشطة',
  openNavigationMenu: 'فتح قائمة التنقل',
  closeNavigationMenu: 'إغلاق قائمة التنقل',
  language: 'اللغة',
  actions: 'الإجراءات',
  save: 'حفظ',
  saveChanges: 'حفظ التغييرات',
  cancel: 'إلغاء',
  confirm: 'تأكيد',
  ok: 'موافق',
  close: 'إغلاق',
  closeDialog: 'إغلاق النافذة',
  edit: 'تعديل',
  delete: 'حذف',
  remove: 'إزالة',
  view: 'عرض',
  add: 'إضافة',
  create: 'إنشاء',
  update: 'تحديث',
  done: 'تم',
  back: 'رجوع',
  next: 'التالي',
  previous: 'السابق',
  submit: 'إرسال',
  print: 'طباعة',
  download: 'تنزيل',
  upload: 'رفع',
  export: 'تصدير',
  filter: 'تصفية',
  filters: 'عوامل التصفية',
  clearFilters: 'مسح عوامل التصفية',
  status: 'الحالة',
  type: 'النوع',
  name: 'الاسم',
  date: 'التاريخ',
  description: 'الوصف',
  notes: 'ملاحظات',
  amount: 'المبلغ',
  total: 'الإجمالي',
  loading: 'جارٍ التحميل...',
  today: 'اليوم',
  yesterday: 'أمس',
  daysAgo: 'قبل {count} أيام',
  pleaseTryAgain: 'يرجى المحاولة مرة أخرى.',
  pleaseCheckFormAndTryAgain: 'يرجى مراجعة النموذج والمحاولة مرة أخرى.',
  failedToGenerateDocument: 'فشل إنشاء المستند',
  failedToSendEmail: 'فشل إرسال البريد الإلكتروني',
  failedToSaveChanges: 'فشل حفظ التغييرات',
  failedToChangeStatus: 'فشل تغيير الحالة',
  uploadFailed: 'فشل الرفع',
  downloadFailed: 'فشل التنزيل',
  changeNotSaved: 'لم يُحفظ التغيير',
  sentTo: 'أُرسل إلى {email}.',
  createdSuccessfully: 'تم إنشاء {no} بنجاح.',
  noResultsFound: 'لا توجد نتائج',
  all: 'الكل',
  optional: 'اختياري',
  yes: 'نعم',
  no: 'لا',
  clearSearch: 'مسح البحث',
  showingZeroResults: 'عرض 0 نتيجة',
  showingRange: 'عرض {start}–{end} من {total}',
  perPage: '{size} / صفحة',
  previousPage: 'الصفحة السابقة',
  nextPage: 'الصفحة التالية',
  previousPageSet: 'الصفحات الخمس السابقة',
  nextPageSet: 'الصفحات الخمس التالية',
  goToPage: 'الانتقال إلى الصفحة {page}',
  pagination: 'الصفحات',
  noRecordsFound: 'لا توجد سجلات',
  noRecordsFoundDescription: 'لا يوجد شيء لعرضه بعد.',
  selectedCount: '{count} محدد',
  clearSelection: 'إلغاء التحديد',
  selectAllRowsOnPage: 'تحديد جميع الصفوف في هذه الصفحة',
  selectRow: 'تحديد الصف {id}',
  dismissAlert: 'إغلاق التنبيه',
  dismissNotification: 'إغلاق الإشعار',
  goToStep: 'الانتقال إلى الخطوة {step}: {label}',
  selectDate: 'اختر تاريخاً',
  selectOption: 'اختر خياراً',
  richText: {
    bold: 'غامق',
    italic: 'مائل',
    underline: 'تسطير',
    fontSize: 'حجم الخط',
    fontSizeHint: 'حجم الخط (حدد النص أولاً)',
    fontSizeSmall: 'صغير',
    fontSizeNormal: 'عادي',
    fontSizeMedium: 'متوسط',
    fontSizeLarge: 'كبير',
    fontSizeXLarge: 'كبير جداً',
    insertImage: 'إدراج صورة',
    startTyping: 'ابدأ الكتابة...',
  },

  signedDocumentUploadDialog: {
    title: 'رفع المستند الموقّع',
    description: 'ارفع نسخة ممسوحة ضوئياً من النسخة الموقّعة فعلياً من العميل للتأكيد.',
    hint: 'ملفات PDF فقط',
    fileRequired: 'ارفع المستند الموقّع للمتابعة.',
    confirm: 'تأكيد',
  },

  mobileBlockScreen: {
    title: 'يفضل استخدام شاشة أكبر',
    description:
      'تطبيق ServiceOS هو تطبيق أعمال متكامل مصمم للعمل التفصيلي جنباً إلى جنب -- مساحات المشاريع والمستندات وتتبع سير العمل غير مصممة لشاشات الجوال الصغيرة.',
    recommendation: 'يرجى المتابعة باستخدام جهاز لوحي أو حاسوب محمول أو حاسوب مكتبي.',
  },

  emailListInput: {
    limitReached: 'تم الوصول إلى الحد الأقصى وهو {max} عناوين بريد إلكتروني.',
    duplicate: 'عنوان البريد الإلكتروني هذا موجود بالفعل في القائمة.',
  },
}
