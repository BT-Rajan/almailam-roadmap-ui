...
// Forms this project's service requires, per the Service Document Map
// (Administration) -- the same list the Overview tab's Required
// Documents checklist uses, so "which forms can be filled here" is one
// definition, not two that could drift apart.
const fillableForms = computed(() =>
  governmentSubmissionStore.forms.filter(
    (form) => form.status === 'Active' && Boolean(form.template) && formMatchesProjectService(form, project.value?.service ?? ''),
  ),
)
