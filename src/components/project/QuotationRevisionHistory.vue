...
const props = withDefaults(
  defineProps<{
    ...
    auditEvents?: QuotationAuditEvent[]
  }>(),
  { auditEvents: () => [] },
)
...
// downloads, prints, emails, and the moment a quotation was approved --
// so staff can see who sent or approved what and when without leaving
// this panel. Every other audit-log row (creation, reopen, etc.) is
// left off; it's implicit in the revision entries themselves.
...
const entries = computed<HistoryEntry[]>(() => {
  ...
  const activityEntries: HistoryEntry[] = props.auditEvents
    .filter((event) => event.action in ACTIVITY_LABEL_KEYS || (event.action === 'Status changed' && event.newValue === 'Approved'))
    .map((event) => ({
      id: `event-${event.id}`,
      date: event.timestamp,
      title: event.action === 'Status changed' ? t('project.revisionHistory.quotationApproved') : t(ACTIVITY_LABEL_KEYS[event.action]),
