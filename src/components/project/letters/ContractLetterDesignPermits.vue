...
/**
 * Bilingual rendering of the "Design & Permits" letter: the Arabic
 * block (verbatim, same wording as the quotation letter) followed by
 * its English translation, in that order, in the same document -- per
 * the confirmed requirement. Structure/boilerplate is fixed; only the
 * fields below are app data or per-document free text.
 */
...
<template>
  <div class="space-y-8">
    ...
    <div dir="rtl" lang="ar" class="font-arabic space-y-4 text-[15px] leading-8 text-slate-900">
      ...
      <p class="text-center">
        {{ feeWordsAr }}
        <span v-if="contract.feeFrequency !== 'Monthly'">
          شامل المصاريف الحكومية وأخذ الموافقات من الجهات المعنية
        </span>
      </p>

      <template v-if="contract.paymentTermsAr.length">
        <p>مقسمة على دفعات تدفع كالتالي:-</p>
        <EditableList
          :model-value="contract.paymentTermsAr"
          :editable="editable"
          @update:model-value="(v) => emit('patch', { paymentTermsAr: v })"
        />
      </template>
    ...
    <!-- English block: translation of the block above, same data -->
    <div dir="ltr" lang="en" class="space-y-4 text-[15px] leading-8 text-slate-900">
      ...
      <div class="border border-slate-800 px-3 py-1 text-center font-bold">Technical Offer:-</div>
      ...
      <p class="text-center">
        {{ feeWordsEn }}
        <span v-if="contract.feeFrequency !== 'Monthly'">
          inclusive of government expenses and obtaining approvals from the relevant authorities
        </span>
      </p>

      <template v-if="contract.paymentTermsEn.length">
        <p>Payable in installments as follows:-</p>
        <EditableList
          :model-value="contract.paymentTermsEn"
          :editable="editable"
          @update:model-value="(v) => emit('patch', { paymentTermsEn: v })"
        />
      </template>
