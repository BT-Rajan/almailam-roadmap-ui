...
describe('describeLoadError', () => {
  it('appends the HTTP status and the backend message for API errors', () => {
    expect(describeLoadError(FALLBACK, new ApiError(429, 'Too many requests. Please slow down.'))).toBe(
      `${FALLBACK} (HTTP 429 – Too many requests. Please slow down.)`,
    )
    expect(describeLoadError(FALLBACK, new ApiError(403, 'You do not have permission to do this.'))).toBe(
  ...
  it('does not repeat the status when the backend sent no message of its own', () => {
    expect(describeLoadError(FALLBACK, new ApiError(500, 'Request failed (500)'))).toBe(`${FALLBACK} (HTTP 500)`)
  })
  ...
  it('uses the network wording as-is when there was no response at all', () => {
    ...
    expect(describeLoadError(FALLBACK, new ApiError(0, offline))).toBe(`${FALLBACK} (${offline})`)
  })
  ...
  it('keeps the friendly text for non-API errors but logs them instead of swallowing them', () => {
    ...
    expect(describeLoadError(FALLBACK, bug)).toBe(FALLBACK)
