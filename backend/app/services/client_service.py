...
def create_client(db: Session, payload, user_id: int | None) -> Client:
    client = Client(
        ...
        # Documents Required -> Verification Required -> Under Review) --
        # the New Client wizard already requires full contact, address,
        # identification and consent details up front, so there is
        # nothing left for that pipeline to gate. They can be selected
        # on a project immediately (see project_service.create_project's
        # onboarding_state == "Ready" check). Staff can still move a
        # client to any of the review states manually afterwards via
        # "Change Status" on the client workspace page if something
        # needs a closer look.
