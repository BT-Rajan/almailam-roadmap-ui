...
def create_authority(payload: AuthorityIn, db: Session = Depends(get_db), _=Depends(can_edit)):
    return AuthorityOut.from_model(government_service.create_authority(db, payload))


@router.patch("/authorities/{authority_id}", response_model=AuthorityOut)
def update_authority(
    authority_id: str, payload: AuthorityIn, db: Session = Depends(get_db), _=Depends(can_edit)
):
    authority = government_service.update_authority(
        db, government_service.parse_authority_id(authority_id), payload
    )
    return AuthorityOut.from_model(authority)


@router.delete("/authorities/{authority_id}", status_code=204)
def delete_authority(authority_id: str, db: Session = Depends(get_db), _=Depends(can_edit)):
    government_service.delete_authority(db, government_service.parse_authority_id(authority_id))
...
def create_form(payload: FormIn, db: Session = Depends(get_db), _=Depends(can_edit)):
    return FormOut.from_model(government_service.create_form(db, payload))


@router.patch("/forms/{form_id}", response_model=FormOut)
def update_form(form_id: str, payload: FormIn, db: Session = Depends(get_db), _=Depends(can_edit)):
    form = government_service.update_form(db, government_service.parse_form_id(form_id), payload)
    return FormOut.from_model(form)


@router.delete("/forms/{form_id}", status_code=204)
def delete_form(form_id: str, db: Session = Depends(get_db), _=Depends(can_edit)):
    government_service.delete_form(db, government_service.parse_form_id(form_id))


@router.patch("/forms/{form_id}/status", response_model=FormOut)
def set_form_status(
    form_id: str, payload: FormStatusUpdate, db: Session = Depends(get_db), _=Depends(can_edit)
):
    form = government_service.set_form_status(db, government_service.parse_form_id(form_id), payload.status)
    return FormOut.from_model(form)
