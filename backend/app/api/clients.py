...
def list_clients(
    ...
    result = client_service.list_clients(
        db, search, clientType, status, onboardingState, accountManagerId, sort, page, pageSize, deleted
    )
...
def create_client_full(
    ...
    document) in one request, immediately, with no staging/confirmation
    step. See client_service.create_client_full. `payload` is a JSON
    string (not a plain Form field per part) since it nests a
    variable-length list of contacts alongside the client/address/
    identification objects -- parsed here, then validated the same way
    any other request body would be.
    """
...
def restore_client(client_id: str, db: Session = Depends(get_db), current_user: User = Depends(can_delete)):
    client = client_service.restore_client(db, client_service.parse_client_id(client_id), current_user.id)
    names = _account_manager_names(db, [client])
    return _client_out(client, names)
